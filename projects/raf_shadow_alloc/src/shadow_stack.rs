...
/// Allocates `size` amount of bytes on shadow stack, which is a thread local
/// static array. And then runs `func` on it.
/// 
/// # Safety
/// It is safe to use, except it doesn't zero the underlying memory before usage.
/// That's why it is marked as unsafe.
/// 
/// # Notes
/// This function can be called in nested manner. Will panic if runs out of
/// shadow stack space.
#[inline]
pub unsafe fn shadow_alloc<F>(size: usize, func: F)
    where F: FnOnce(&mut [u8])
{
    #[inline(always)]
    fn dummy(_: &mut [u8]) {}

    shadow_alloc!(size, dummy, func);
}

/// Allocates `size` amount of bytes on shadow stack, which is a thread local
/// static array. And then runs `func` on it. Zeros the memory before usage.
/// 
/// # Notes
/// This function can be called in nested manner. Will panic if runs out of
/// shadow stack space.
#[inline]
pub fn shadow_alloc_zeroed<F>(size: usize, func: F)
    where F: FnOnce(&mut [u8])
{
    #[inline(always)]
    fn zero(buf: &mut [u8]) { buf.fill(0); }

    shadow_alloc!(size, zero, func);
}
