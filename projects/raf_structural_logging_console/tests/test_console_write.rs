...
fn test1() {
    ...
    let template = TMPL_BUILDER.create_from_str("test");
...
fn test2() {
    let log_level = LogLevel::Info;
    let template = TMPL_BUILDER.create_from_str("[{created_at}] [{log_level}] [{dur}] {baz} = {xyz}... {arr}");
    let template_clone = template.clone();
    ...
    let template2 = TMPL_BUILDER.create_from_str("[{created_at}] this is dict: {dct}  ");
