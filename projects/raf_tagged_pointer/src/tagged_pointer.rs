...
impl<T: ?Sized, const BIT_COUNT: usize> TaggedPointer<T, BIT_COUNT> {
    ...
    /// 
    /// # Errors
    /// * [`TaggedPointerNewError::PointerMisaligned`] if `raw_ptr` is
    ///   incorrectly aligned to `1 << BIT_COUNT` bytes.
    pub fn new(raw_ptr: *mut T) -> Result<Self, TaggedPointerNewError> {
        let repr_ptr = unsafe_size_it!(raw_ptr);
        if (*repr_ptr & Self::MASK) != 0 {
            Err(TaggedPointerNewError::PointerMisaligned)
        }
        else
        {
